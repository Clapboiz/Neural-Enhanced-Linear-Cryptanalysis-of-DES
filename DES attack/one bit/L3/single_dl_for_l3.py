...
def make_train_data(n,num_pair):
    ...
    # Build training samples in chunks to avoid large temporary arrays in key schedule.
    chunk_size = 2**18
    cipher_X = np.empty((n, num_pair), dtype=np.uint8)
    for start in range(0, n, chunk_size):
        end = min(start + chunk_size, n)
        cur_n = end - start
        print("data chunk:", start, "->", end)

        keys = np.frombuffer(urandom(64*cur_n), dtype=np.uint8).reshape(-1,64)
        keys = keys & 1
        subkey=ciph.expand_key(keys,Round)

        for i in range(num_pair):
            plain = np.frombuffer(urandom(64*cur_n), dtype=np.uint8).reshape(-1,64)
            plain = plain & 1
            ...
            cipher_X[start:end, i] = (
                Ph[:,31-7]^Ph[:,31-18]^Ph[:,31-24]^Ph[:,31-29]^Pl[:,31-15]^
                Ch[:,31-7]^Ch[:,31-18]^Ch[:,31-24]^Ch[:,31-29]^Cl[:,31-15]^
                subkey[0][:,47-22]^subkey[2][:,47-22]
            )
